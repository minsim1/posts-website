...
export async function GET({ url, locals }: RequestEvent) {
	...
	try {	
		...
		const postCount = await PostManager.GetNumOfAnonPostsWithPresentAuthorIdBeforeTimestamp(
			postDeletionCutoffTimestamp
		)

		const commentCount = await CommentManager.GetNumOfAnonCommentsWithPresentAuthorIdBeforeTimestamp(
			commentDeletionCutoffTimestamp
		)

		const modLogCount = await UserManager.GetNumOfModLogsOlderThan(
			modLogDeletionCutoffTimestamp
		)

		const response: APITypes.Admin.Records.QueryResponse = {
			queryResult: {
				numOfAnonCommentsWithNonExpungedAuthorIDs: commentCount,
				numOfAnonPostsWithNonExpungedAuthorIDs: postCount,
				numOfModLogsToBeDeleted: modLogCount
			}
		}
